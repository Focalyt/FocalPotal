	...
	.get(auth1, async (req, res) => {
		try {
			...
			if (Array.isArray(candidates)) {
				for (let candidate of candidates) {
				  const courseDocsRequired = candidate.docsRequired || [];
				  const uploadedDocs = candidate.uploadedDocs || []; // ✅ now direct
				  ...
				  if (Array.isArray(uploadedDocs)) {
					for (let reqDoc of courseDocsRequired) {
					  const uploadedDoc = uploadedDocs.find(
						(doc) => doc.docsId && doc.docsId.toString() === reqDoc._id.toString()
					  );
					...
					// Agar uploadedDocs hi nahi hai
					pendingForUpload = totalRequired;
				  }
			  
				  const uploaded = verified + pending;
				  const percent = totalRequired > 0
					? Math.round((uploaded / totalRequired) * 100)
					: 0;
...
.get(auth1, async (req, res) => {
	try {
	  ...
	  // ✅ अब AppliedCourses से directly fetch कर रहे हैं
	  ...
	  let uploadedDocs = [];
	  if (appliedCourse && appliedCourse.uploadedDocs.length > 0) {
		uploadedDocs = appliedCourse.uploadedDocs;
	  }
  
	  const docsRequired = course.docsRequired || [];
  
	  const mergedDocs = docsRequired.map(reqDoc => {
	  ...
	  const countDocs = (uploadedDocs) => {
		const verifiedDocs = uploadedDocs.filter(doc => doc.status === "Verified").length;
		const pendingDocs = uploadedDocs.filter(doc => doc.status === "Pending").length;
		const rejectedDocs = uploadedDocs.filter(doc => doc.status === "Rejected").length;
		return {
	  ...
	  const courseWiseDocumentCounts = countDocs(uploadedDocs);
  ...
  .put(async (req, res) => {
	try {
	  ...
	  const verifiedBy = req.session?.user?._id;
	  if (!verifiedBy) {
		return res.status(400).json({ message: "Missing verifiedBy fields" });
	  }
	  ...
	  // 🔍 Step 1: Find the AppliedCourses document
	  const appliedCourse = await AppliedCourses.findOne({
		_candidate: candidateId,
		_course: courseId
	  });
	  ...
	  if (!appliedCourse) {
		return res.status(404).json({ message: "Applied Course not found" });
	  }
  
	  // 🔍 Step 2: Find the document inside uploadedDocs
	  const document = appliedCourse.uploadedDocs.find(doc => doc._id.toString() === objectId);
	  ...
	  // ✅ Step 4: Save the updated AppliedCourses document
...
.post(auth1, async (req, res) => {
	try {
	  ...
	  if (!candidate) {
		return res.status(400).json({ error: "Candidate not found with this mobile number." });
	  }
	  ...
	  filesArray.forEach((item) => {
		...
		const key = `Documents for course/${courseId}/${candidate._id}/${docsId}/${uuid()}.${ext}`;
		const params = {
		  Bucket: bucketName,
		  Key: key,
		  Body: item.data,
	  ...
	  // Now update inside AppliedCourses
	  const updatedAppliedCourse = await AppliedCourses.findOneAndUpdate(
		{ _candidate: candidate._id, _course: courseId },
		{
		  $push: {
			uploadedDocs: {
